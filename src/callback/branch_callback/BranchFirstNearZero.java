...
public class BranchFirstNearZero extends BranchCallback{
	...
	protected void main() throws IloException {
		...
		int nb = 2; // TODO  it should be always 2 ? It can be 3?
		...
		int better_j = -1;
		boolean ok = false;
		for(int i = 0 ; i < myp.n() ; ++i){
			for(int j = i+1 ; j < myp.n() ; ++j){

				if(this.getFeasibility(myp.v_edge[i][j]).equals(IntegerFeasibilityStatus.Infeasible)){
					double value = this.getValue(myp.v_edge[i][j]);
					if(value<0.08 && value>0 && this.myp.d[i][j]>0){
						better_i = i;
						better_j = j;
						ok = true;
						break;
